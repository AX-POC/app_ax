...
const agents = [
  ...
  },
  {
    ...
    suggestions: ['?§Ìéò???¨Ïù¥???¥Ïñ¥Ï§?, 'Deploy UK Store']
  },
...
function OrderIssuesForm({ onResolveErrors, onNotifyScm }: { onResolveErrors: (ids: string[]) => void, onNotifyScm: (ids: string[]) => void }) {
  ...
  return (
    <div className="chat-form" style={{ background: '#fafafa', border: '1px solid #ced4da', animation: 'fadeIn 0.4s ease' }}>
      <p style={{ margin: '0 0 16px 0', fontSize: '0.85rem', fontWeight: 'bold' }}>?†Ô∏è Pending Order Flow Actions</p>
      
      {/* 1. Creation Errors */}
      <div style={{ marginBottom: '16px', background: '#fff', border: '1px solid #e9ecef', borderRadius: '4px', padding: '12px' }}>
        <h4 style={{ margin: '0 0 12px 0', fontSize: '0.8rem', color: '#dc3545', display: 'flex', alignItems: 'center', gap: '6px' }}>?î¥ Order Creation Error</h4>
          ...
          <div style={{ background: '#212529', color: '#f8f9fa', padding: '10px', borderRadius: '4px', fontSize: '0.7rem', fontFamily: 'monospace', marginBottom: '12px', whiteSpace: 'pre-wrap', maxHeight: '120px', overflowY: 'auto', position: 'relative' }}>
            ...
            {isAnalyzing && (
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(33,37,41,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#adb5bd', fontSize: '0.75rem', fontWeight: 'bold' }}>
                <span className="heartbeat-animation" style={{ marginRight: '8px' }}>?§ñ</span> AI Analysis in progress...
              </div>
            )}
          </div>
        )}
        ...
        {showAiProposal && !isAnalyzing && checkedErrors.length > 0 && (
          <div style={{ background: '#f8f9fa', border: '1px solid #ced4da', borderLeft: '4px solid #0d6efd', padding: '12px', borderRadius: '4px', marginBottom: '12px', animation: 'fadeIn 0.5s ease' }}>
            <h5 style={{ margin: '0 0 8px 0', fontSize: '0.8rem', color: '#0d6efd', display: 'flex', alignItems: 'center', gap: '6px' }}>?í° Agent Analysis</h5>
            {!isCarrierFixed ? (
              <>
                <p style={{ margin: '0 0 10px 0', fontSize: '0.75rem', color: '#495057', lineHeight: '1.4' }}>
                  ?§Î•∏ ?úÌíà?§Í≥º ÎπÑÍµê?àÏùÑ ?? Ï∫êÎ¶¨??Í∞íÏù¥ ?ÑÎùΩ?òÏñ¥ ?àÏäµ?àÎã§.<br/>
                  ?ôÏùº ?úÌíàÍµ∞Ïùò Carrier Î™ÖÏù∏ <strong>Pantos</strong>Î°??∏ÌåÖ?†Íπå??
                </p>
                <button 
                  className="btn" 
      ...
      <div style={{ background: '#fff', border: '1px solid #e9ecef', borderRadius: '4px', padding: '12px' }}>
        <h4 style={{ margin: '0 0 12px 0', fontSize: '0.8rem', color: '#ffc107', display: 'flex', alignItems: 'center', gap: '6px' }}>?ü° Order Picking Delayed</h4>
...
export default function AiAgentSidebar() {
  ...
  // ?êÏù¥?ÑÌä∏ ID ???∞Ï∏° Admin ?òÏù¥ÏßÄ Îß§Ìïë
          ...
          if (state.logs && state.logs.length > 0) {
            ...
            // ?àÎ°≠Í≤?Î∞úÏÉù???¨Í∞Å???ÑÌòë Í∞êÏ? (RESOLVEDÍ∞Ä ?ÑÎãå Í≤ΩÏö∞)
              ...
              if (secAgent) {
                ...
                // 2. Í≥µÍ≤© ?êÏ? Î©îÏãúÏßÄ Ï∂úÎ†•
                setMessages([
                  { role: 'agent', type: 'text', content: `?î¥ **CRITICAL ALERT:**\nAnomaly detected! A massive spike in malicious traffic (${latestLog.type}) is hitting the endpoints from multiple IP ranges.` }
                ]);
                
                // Ï≤òÎ¶¨ Ï§??†ÎãàÎ©îÏù¥???®Í≥º)Î•??ÑÌï¥ ?Ä?¥Ìïë ?∏ÎîîÏºÄ?¥ÌÑ∞ Ïº?
                setTimeout(() => setIsTyping(true), 1000);
                
                // 3. ?êÏú® Î∞©Ïñ¥ Ï°∞Ïπò Î©îÏãúÏßÄ Î∞?Ï¢ÖÍ≤∞ Ï≤òÎ¶¨ (10Ï¥???
                setTimeout(async () => {
                  setIsTyping(false);
  ...
  const handleSelectAgent = (agent: any) => {
    // ?∞Ï∏° ?Ä?úÎ≥¥?úÎ? ?êÏù¥?ÑÌä∏??ÎßûÎäî Admin ?òÏù¥ÏßÄÎ°??¥Îèô
  ...
  const submitMessage = async (text: string) => {
    ...
    if (activeAgent?.id === 'security') {
      ...
      try {
        if (lower.includes('ddos')) {
          ...
          const state = await res.json();
          setTimeout(() => { setIsTyping(false); setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?åä DDoS attack simulation executed.\n\n?î¥ Threat Level: ${state.level}\n??Active Threats: ${state.activeThreats}\n?õ°Ô∏?Blocked Today: ${state.blockedToday}\n?ìä Total Scanned: ${state.totalScanned.toLocaleString()}\n\nThe Security Dashboard on the right has been updated in real-time.` }]); }, 800);
          ...
          const res = await fetch(`${COMMERCE_API}/api/security/state`);
          const state = await res.json();
          setTimeout(() => { setIsTyping(false); setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?íâ SQL Injection attack simulation executed.\n\n?î¥ Threat Level: ${state.level}\n??Active Threats: ${state.activeThreats}\n?õ°Ô∏?Blocked Today: ${state.blockedToday}\n\nInjection attempt has been intercepted and logged.` }]); }, 800);
          ...
          const res = await fetch(`${COMMERCE_API}/api/security/state`);
          const state = await res.json();
          setTimeout(() => { setIsTyping(false); setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?îì Brute Force attack simulation executed.\n\n?î¥ Threat Level: ${state.level}\n??Active Threats: ${state.activeThreats}\n?õ°Ô∏?Blocked Today: ${state.blockedToday}\n\nSuspicious login attempts have been blocked.` }]); }, 800);
        } else if (lower.includes('price') || lower.includes('Í∞ÄÍ≤?)) {
          ...
          const res = await fetch(`${COMMERCE_API}/api/security/state`);
          const state = await res.json();
          setTimeout(() => { setIsTyping(false); setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?í∞ Price Manipulation attack simulation executed.\n\n?î¥ Threat Level: ${state.level}\n?õ°Ô∏?Blocked Today: ${state.blockedToday}\n\nPrice tampering attempt detected and neutralized.` }]); }, 800);
        } else if (lower.includes('full') || lower.includes('all') || lower.includes('?ÑÏ≤¥')) {
          await fetch(`${COMMERCE_API}/api/security/simulate-all`, { method: 'POST' });
          setTimeout(async () => {
            const res = await fetch(`${COMMERCE_API}/api/security/state`);
            const state = await res.json();
            setIsTyping(false);
            setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?ö® Full Attack Scenario executed!\n\n?î¥ Threat Level: ${state.level}\n??Active Threats: ${state.activeThreats}\n?õ°Ô∏?Blocked Today: ${state.blockedToday}\n?ìä Total Scanned: ${state.totalScanned.toLocaleString()}\n?ìã Total Logs: ${state.logs.length}\n\nAll attack vectors have been simulated. Check the dashboard for details.` }]);
          }, 2000);
        } else if (lower.includes('report') || lower.includes('Î≥¥Í≥†??)) {
          // Î≥¥Í≥†???ùÏóÖ Î™®Îã¨ ?ÑÏö∞Í∏?
          setIsTyping(false);
          setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '?ìä Generating AI Security Report... Opening report window.' }]);
        ...
        } else if (lower.includes('clear') || lower.includes('Ï¥àÍ∏∞??) || lower.includes('reset')) {
          await fetch(`${COMMERCE_API}/api/security/logs`, { method: 'DELETE' });
          setTimeout(() => { setIsTyping(false); setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?óëÔ∏?Security logs cleared. System reset to SAFE status.\n\nAll threat data has been purged. Dashboard is now clean.` }]); }, 500);
        } else if (lower.includes('status') || lower.includes('level') || lower.includes('?ÅÌÉú') || lower.includes('threat')) {
          const res = await fetch(`${COMMERCE_API}/api/security/state`);
          const state = await res.json();
          setTimeout(() => { setIsTyping(false); setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?õ°Ô∏?Current Security Status:\n\n??Threat Level: ${state.level}\n??Active Threats: ${state.activeThreats}\n??Blocked Today: ${state.blockedToday}\n??Total Scanned: ${state.totalScanned.toLocaleString()}\n??Event Logs: ${state.logs.length} entries\n??Last Updated: ${new Date(state.lastUpdated).toLocaleTimeString('ko-KR')}` }]); }, 500);
        } else {
          // AI ?Ä?îÎ°ú fallback
          try {
        ...
        setIsTyping(false);
        setReportLoading(false);
        setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '?†Ô∏è Failed to connect to Security Engine. Please check your network connection.' }]);
      }
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?†Ô∏è ${data.error || '?????ÜÎäî ?úÎ≤Ñ ?§Î•ò'}` }]);
        return;
      }
  ...
  const handleStopPromotion = async (id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setMessages(prev => [...prev, { role: 'agent', type: 'text', content: '?†Ô∏è Failed to stop the promotion. Please try again.' }]);
      }
    } catch (err: any) {
      console.error("Failed to stop promotion:", err);
      setMessages(prev => [...prev, { role: 'agent', type: 'text', content: `?†Ô∏è Failed to stop promotion: ${err.message}` }]);
    }
  };
  ...
  const handleApproveAction = async (details: any) => {
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'agent',
        type: 'text',
        content: `?†Ô∏è Deployment Failed: ${err.message}`
      }]);
    }
  };
          ...
          >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                    <span style={{ fontSize: 22 }}>?õ°Ô∏?/span>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
              {reportLoading ? (
                <div style={{ textAlign: 'center', padding: 60, color: '#64748b' }}>
                  <div style={{ fontSize: 28, marginBottom: 12, animation: 'dotPulse 1.4s infinite ease-in-out' }}>?§ñ</div>
                  <div style={{ fontSize: 15, fontWeight: 600 }}>AIÍ∞Ä Î≥¥Ïïà Î°úÍ∑∏Î•?Î∂ÑÏÑù?òÍ≥† ?àÏäµ?àÎã§...</div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 11, color: '#94a3b8' }}>
                ?îí Confidential ¬∑ LG AI Commerce Security Team
              </div>
          ...
          {agents.map(agent => (
            <div key={agent.id} className="ai-agent-item" style={{ display: 'flex', alignItems: 'center' }} onClick={() => handleSelectAgent(agent)}>
              ...
              {agent.id === 'promotion' && (
                ...
                >
                  <span style={{ fontSize: '1.2rem' }}>?îî</span>
                ...
                <div key={idx} className={`chat-bubble ${msg.role}`}>
                  ...
                  {msg.type === 'progress' && (
                    <div className="progress-card" style={{ background: '#f8f9fa', border: '1px solid #dee2e6', padding: '12px', borderRadius: '8px', marginTop: '8px' }}>
                      <h4 style={{ margin: '0 0 10px 0', fontSize: '0.9rem', color: '#495057' }}>{msg.progressTitle || '?ôÔ∏è Deployment Progress'}</h4>
                  ...
                  {msg.type === 'action' && msg.actionDetails?.type === 'DATA_VISUALIZATION_OLED' && (
                    <div className="action-card" style={{ background: '#f8f9fa', border: '1px solid #dee2e6' }}>
                      <div className="action-card-title" style={{ fontSize: '0.9rem', color: '#495057', display: 'flex', alignItems: 'center', gap: '8px' }}>
                         ?ìä <span>7-Day Funnel Analysis (OLED65G4)</span>
                      </div>
                  ...
                  {msg.type === 'action' && msg.actionDetails?.type === 'ROLLOUT_SUCCESS' && (
                    <div className="action-card" style={{ borderLeft: '4px solid var(--accent)' }}>
                      <div className="action-card-title">{msg.actionDetails.labels?.cardTitle || '?åç Site Launch Ready'}</div>
                      <div className="action-card-details">
                      ...
                      <div className="action-row">
                        ...
                        <button className="action-btn approve" style={{ background: 'var(--accent)' }} onClick={() => {
                          ...
                          const pTitle = config.labels?.progressTitle || '?ôÔ∏è Deployment Progress';
                          setMessages(prev => [...prev, 
                          ...
                          eventSource.addEventListener('complete', (e) => {
                             ...
                             showCenterSuccessToast(`?éâ ${config.region} Rollout Successfully Completed!`);
                  ...
                  {msg.type === 'action' && msg.actionDetails?.type === 'STOP_PROMOTION' && (
                    <div className="action-card">
                       <span style={{color: 'var(--lg-red)', fontWeight: 'bold'}}>
                         ?õë {(msg.actionDetails.schedule || 'Immediate').toLowerCase().includes('immediate') ? `Stopped Promotion ID: ${msg.actionDetails.promotionId.replace(/\[|\]/g, '')}` : `Scheduled Stop: [${msg.actionDetails.promotionId.replace(/\[|\]/g, '')}] on ${msg.actionDetails.schedule}`}
                       </span>
                    </div>
                  )}
                  ...
                  {msg.type === 'action' && msg.actionDetails?.type === 'PTO_CREATE' && (
                    <div className="action-card" style={{ borderLeft: '4px solid var(--lg-red)', background: '#fff4f4' }}>
                      <div className="action-card-title">?ì¶ PTO Bundle Draft</div>
                      <div className="action-card-details">
                  ...
                  {msg.type === 'action' && msg.actionDetails?.type === 'ASK_PUBLISH_LIVE' && (
                    <div className="action-card" style={{ background: '#fff9f9', borderLeft: '4px solid var(--lg-red)' }}>
                      ...
                      <div className="action-card-content">
                        <p style={{ marginBottom: '16px' }}>The bundle <strong>{msg.actionDetails.name}</strong> is now visible in the preview storefront. If everything looks correct, you can publish it live.</p>
                      </div>
                      <div className="action-row">
                        <button className="action-btn secondary" style={{ background: '#475569', color: 'white', border: 'none' }} onClick={() => {
                          localStorage.setItem('pending-ai-promo', JSON.stringify({
                  ...
                  {msg.type === 'list_promotions' && (
                    <div className="action-card" style={{ background: 'var(--lg-body-bg)' }}>
                      <div className="action-card-title">?ìù Active Promotions ({activePromotions.length})</div>
...
function PtoCreationForm({ onSubmit }: { onSubmit: (name: string, sku: string, price: number, skus: string[], category: string, images: string[], startDate: string, endDate: string) => void }) {
  ...
  const [fullCatalog, setFullCatalog] = useState<Record<string, {sku: string, name: string, defaultPrice: number, imageUrl: string}[]>>({
    'Accessories': [
  ...
  useEffect(() => {
    fetch('/api/products?size=200')
      ...
      .then(data => {
        ...
        const catalog: Record<string, {sku: string, name: string, defaultPrice: number, imageUrl: string}[]> = {
          'Accessories': [],
          'Air Solutions': [],
          'Appliance': [],
          'Computing': [],
        ...
        data.content.forEach((p: any) => {
          ...
          if (name.includes('tv') || name.includes('sound') || name.includes('audio') || name.includes('ray') || name.includes('oled') || name.includes('qned') || name.includes('nanocell') || sku.includes('qned') || sku.includes('oled')) {
             cat = 'TV-Audio-Video';
          } else if (name.includes('air') || name.includes('purifier') || name.includes('acondicionado') || name.includes('conductos') || name.includes('split') || name.includes('heat')) {
             cat = 'Air Solutions';
          } else if (name.includes('monitor') || name.includes('laptop') || name.includes('gram') || sku.includes('z90')) {
             cat = 'Computing';
          } else if (name.includes('remote') || name.includes('mount') || name.includes('stand') || sku.includes('mr22') || sku.includes('wb21')) {
             cat = 'Accessories';
          }
      ...
      .catch(err => console.error('Failed to load full catalog:', err));
  }, []);
  ...
  const getFlatCatalog = () => {
     ...
     return [...Object.values(fullCatalog).flat(), ...currentPageProducts, ...Object.values(retainedProducts)];
  };
  ...
  const displayedCatalog = activeTab === 'current' ? currentPageProducts : fullCatalog[browseCategory];
    ...
    <div className="chat-form" style={{ borderLeft: '3px solid var(--lg-red)', background: '#fff4f4' }}>
      ...
      {activeTab === 'browse' && (
         <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
           ...
           >
              {Object.keys(fullCatalog).map(cat => (
